...
var GITHUB_URL = 'https://api.github.com/repos/%s/%s/releases';
...
module.exports = function bowerLatest (packageName, options, cb) {
  ...
  findBowerPackage(packageName, function (bowerPackage) {
    ...
    https.get(options, function(res) {
      ...
      res.on('end', function() {
        var releases = JSON.parse(chunks);
        var component;
        for (var i = 0; i < releases.length; i++) {
          var release = releases[i]
          if (!release.prerelease && !release.draft) {
            
            // Remove the 'v' prefix. e.g.. v0.4.2 -> 0.4.2
            var version = (release.tag_name[0] === 'v') ?
              release.tag_name.slice(1) : release.tag_name;
